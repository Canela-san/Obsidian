...
## 1. Formatos de Instrução

A arquitetura utiliza uma palavra de instrução fixa de **18 bits** (bits `[17:0]`), dividida em três formatos principais: R, I e J.
### Formato R (Tipo Registrador)

Utilizado para operações lógicas e aritméticas entre registradores.

| **Campo**  | **Bits**  | **Tamanho** | **Descrição**                               |
| ---------- | --------- | ----------- | ------------------------------------------- |
| **opcode** | `[17:15]` | 3 bits      | Código da operação principal                |
| **rs**     | `[14:12]` | 3 bits      | Endereço do registrador fonte 1             |
| **rt**     | `[11:9]`  | 3 bits      | Endereço do registrador fonte 2             |
| **rd**     | `[8:6]`   | 3 bits      | Endereço do registrador de destino          |
| **shamt**  | `[5:3]`   | 3 bits      | Quantidade de deslocamento (_Shift amount_) |
| **funct**  | `[2:0]`   | 3 bits      | Código da função específica                 |


### Formato I (Tipo Imediato)

Utilizado para instruções que carregam valores constantes, acesso à memória e desvios condicionais.

| **Campo**       | **Bits**  | **Tamanho** | **Descrição**                              |
| --------------- | --------- | ----------- | ------------------------------------------ |
| **opcode**      | `[17:15]` | 3 bits      | Código da operação principal               |
| **rs**          | `[14:12]` | 3 bits      | Endereço do registrador base/fonte         |
| **rt**          | `[11:9]`  | 3 bits      | Endereço do registrador destino/comparação |
| **x**           | `[8]`     | 1 bit       | _Don't care_ (Bit ignorado)                |
| **address/imm** | `[7:0]`   | 8 bits      | Valor imediato ou endereço                 |

### Formato J (Tipo Salto)

Utilizado exclusivamente para desvios incondicionais (Jump).

| **Campo**   | **Bits**  | **Tamanho** | **Descrição**                 |
| ----------- | --------- | ----------- | ----------------------------- |
| **opcode**  | `[17:15]` | 3 bits      | Código da operação principal  |
| **x**       | `[14:8]`  | 7 bits      | _Don't care_ (Bits ignorados) |
| **address** | `[7:0]`   | 8 bits      | Endereço de destino do salto  |
...
| Negação                   |   `000` / `100`    | `not rd, rs`       | $rd = \neg rs$                |      R      |
| Menor que                 |   `000` / `101`    | `slt rd, rs, rt`   | se $rs < rt$, $rd = 1$        |      R      |
| Deslocamento à esquerda   |   `000` / `110`    | `sll rd, rs, N`    | $rd = rs \ll N$               |      R      |
| Deslocamento à direita    |   `000` / `111`    | `srl rd, rs, N`    | $rd = rs \gg N$               |      R      |
| Carregar dados da memória |   `001` / `xxx`    | `lw rt, rs(N)`     | $rt = \text{Memória}[rs + N]$ |      I      |
| Salvar dados na memória   |   `010` / `xxx`    | `sw rt, rs(N)`     | $\text{Memória}[rs + N] = rt$ |      I      |
...
#### Hexadecimal:
| 0x20205 | 0x2040a | 0x14c0 | 0x2281 | 0x1a006 | 0x38002 | 0x10600 |
| ------- | ------- | ------ | ------ | ------- | ------- | ------- |
|         |         |        |        |         |         |         |

#### Exemplo de compilador em python:
```python
import sys
import re
